...
- **Cookie 封存**：格式 `v1.<iv>.<ciphertext>`（AES-256-GCM）。密钥取 `COOKIE_SECRET`，缺省回退 `JWT_SECRET` 派生，**解密时两个都试** —— 否则后加 `COOKIE_SECRET` 会把旧记录锁死。与 `web/backend/src/utils/cookieVault.js` 交叉验证，两边产出的密文能互解
- **学号比对 / 预置职位白名单 / 权限白名单**：与 `web/backend/src/utils/permissions.js`、`identity.js` 同源，不另起一套
