...
- **离线**：页面壳由 Service Worker 缓存（`sw.js`，含预热的各页面与静态资源），断网也能打开；`/api/*` 刻意不走 Service Worker —— 数据与登录态必须走网络。App 里另有原生层兜底（页面、静态资源与接口都缓存，见安卓端「离线可用」），浏览器里没有这一层，所以断网提示分两种文案（见 `web/README.md`）
...
- **离线壳也在原生层**（真机反馈后补上）：第一版只兜接口，结果「杀进程重开后断网」时**除了主页都打不开、主页也拉不到数据** —— 页面壳那时只靠 `sw.js`，而它在 WebView 冷启动离线时没接管 iframe 的导航（不重启 App 就正常，因为页面早就渲染好了，所以当时看不出来）。现在页面与静态资源（`OfflineApi.SHELL_PATHS`，与 `sw.js` 的 `PRECACHE` 保持同一份清单）同样「在线回源 + 顺手存一份、断网回放」，且**这条路径不看 token** —— 没登录 + 断网也得能打开登录页，否则彻底进不去。断网又没缓存过的导航请求，给一个内置提示页，而不是把 WebView 默认的错误页（一屏英文 +「找不到页面」）甩给用户；只有「打开页面」的请求才给，缺图标 / 脚本时塞 HTML 只会更乱。接口那一侧也顺手改了：**断网回放不再要求 token**，读的是本机已经存下的数据，登录态在不在都不影响，以前先卡 token 会让「冷启动 + 断网 + 探针还没把 token 报上来」白跑一趟
