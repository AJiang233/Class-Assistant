...
/**
 * 小组件的系统广播触发器。两个自定义 action（零点换天、上课每分钟重绘）已拆到
 * WidgetAlarmReceiver（exported=false）—— 它们若留在这里，等于让任何第三方应用都能
 * 向这个 exported=true 的接收器广播触发重绘 / 重排闹钟 / 排一轮同步（issue #84 项 14）。
 *
 ...
 * 换天主力是 Scheduler.scheduleMidnightRefresh 排的本地零点闹钟（见 WidgetAlarmReceiver），
 * 这里处理两类系统时机：
 *
 *   1. 系统时间被改 / 时区变了 —— RTC 闹钟按绝对毫秒存，时区一变「09:50 上课」的含义就变了，
 *      所以这里连课程闹钟也一起重排
 *   2. 应用被覆盖安装 —— 闹钟与 AllowedAlarms 都会被清掉，得重新排
 *
 * 特意**没有**注册 DATE_CHANGED：它不在系统的隐式广播豁免名单里，
 * 清单注册的接收器收不到（Android 8 起隐式广播不再投递给静态接收器）。
 * 换天只能靠零点闹钟兜，这也是那边用闹钟而不是等系统广播的原因。
 */
