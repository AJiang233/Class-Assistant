...
private class EventsWidgetFactory(private val context: Context) :
    ...
    override fun getViewAt(position: Int): RemoteViews? {
        ...
        // 活动缓存里**没有下课时间**（Event 只有 start），所以这里只能写一个点，
        // 也没有进度条可言 —— widget_card_item 的进度条默认就是 gone，不用管。
        views.setTextViewText(R.id.card_item_time, formatClock(row.optLong("start", 0L)))
