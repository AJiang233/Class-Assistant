...
- **管理员面板**：`admin.html`，左栏「添加通知 / 添加活动 / 添加表单 / 管理表单」、右栏「管理成员 / 添加成员 / 管理职位 / 添加职位」八个默认折叠区块（按权限显示）；发布类卡片按 `content:write` 显示，成员卡片按 `user:manage` 显示，职位两张卡片对任何登录用户只读展示（增删在提交时由后端校验权限，无权限返回 403）；成员/职位编辑用弹窗，成员列表中的职务以徽章样式呈现。**添加 / 修改成员都能代填邮箱（选填），邮箱栏标题后带验证状态徽章**：管理员填的或改的地址**一律是「未验证」**（`email_verified` 打回 0）—— 邮箱是找回密码的凭据，只能由本人收码确认，管理员能填地址但代认不了；徽章跟着输入实时变（原样提交就照原状态显示，一改就变「未验证」），所以只改姓名不会把已验证的人误打回未验证（后端按归一化后的值比，大小写不同不算改动）
