...
├── account.html                # 个人中心 —— 资料（联系方式自助修改）/ 偏好设置（主题外观 + 通知；通知走网页推送，App 壳内不显示，由壳的本地通知承担）/ 日历订阅 / 安装到桌面（仅 iOS 与 PC 显示，安卓 / 鸿蒙已有原生 App）/ 开发功能（清缓存刷新 + 推送测试 + 本机通知与上次同步状态，状态经 JS 桥取，网页版没有桥则留空）/ 修改密码 / 关于软件（版本号 / 检查更新）/ 退出登录
...
- **个人中心**：资料（联系方式可自助修改，更新时间按北京时间显示）、偏好设置（主题外观 / Web 端通知 / **课程提醒**——最后一项仅 App 壳，见下）、日历订阅（可自定义提醒提前量/时间范围/是否含通知，并可重置密钥）、开发功能（清缓存刷新 + 活动/通知推送测试 + 一行本机状态：通知开关与上次同步时间，数据来自 `CAHost.appStatus()`，网页版没有这个桥就整行留空；通知被系统关掉时推送测试会直接说明原因而不是假装成功）、安装到桌面（仅 iOS 与 PC 显示，安卓 / 鸿蒙已有原生 App 不引导）、修改密码、关于软件（版本号 + 检查更新 + 项目仓库 + 协作方；版本号取自原生桥，检查更新先问原生桥 `CAHost.platform()` 自己是什么端、再读站点根目录 `version.json` 里对应那一段）、退出登录（红色警示卡）
  - **课程提醒**（`#courseCard`）：整块默认 `hidden`，只有检测到 `CAHost.courseReminderSettings` 才显示 —— 上课提醒要到点弹出，靠的是原生 `AlarmManager` 排期，网页版没有这个能力，给个点了没反应的开关不如不给。设置存在原生侧，网页只读写；桥返回 `{"lead":15,"atStart":true,"courseCount":23}`，其中 `courseCount` 是为了区分「今天没课」与「本机根本没有课表数据」——用户把开关打开却什么都没发生，得能看出是哪一种。改完即存（无保存按钮），原生侧立刻重排闹钟
