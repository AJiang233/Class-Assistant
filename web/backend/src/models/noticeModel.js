...
export class NoticeModel {
   ...
   */
  async create(data) {
    const { title, content, publish_time, publisher, remind_people = null, source = 'manual' } = data;
    const result = await this.db.prepare(
      `INSERT INTO notices (title, content, publish_time, publisher, remind_people, source)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(title, content, publish_time, publisher, remind_people, source).run();
    return result;
  }
  ...
  async list(limit = 50, offset = 0) {
    const result = await this.db.prepare(
      `SELECT id, title, content, publish_time, publisher, remind_people, source, created_at
       FROM notices
